...
func Test_NewSimpleGuardGT(t *testing.T) {
	type args struct {
		id        string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			instance := g.NewSimpleGuardGT(tt.args.id, tt.args.reference)
			a.NotNil(t, instance, "instance should not be nil")
		})
	}
}

func Test_SimpleGuardGT_Evaluate(t *testing.T) {
	type args struct {
		value int
	}
	...
	intGuard := g.NewSimpleGuardGT[int]("key1", intRef)
	emptyGuard1 := g.NewSimpleGuardGT[int]("key1", nil)
	emptyGuard2 := g.NewSimpleGuardGT[int]("key1", emptyRef)

	testCases := []struct {
		name     string
		args     args
		expected bool
		guard    *g.SimpleGuardGT[int]
	}{
		{"case 01", args{1}, true, &emptyGuard1},
		{"case 02", args{1}, true, &emptyGuard2},
		{"case 03", args{1}, false, &intGuard},
		{"case 04", args{2}, true, &intGuard},
		{"case 04", args{0}, false, &intGuard},
	}
...
func Test_SimpleGuardGT_EvaluateWithErr(t *testing.T) {
	type args struct {
		value int
	}
	...
	testCases := []struct {
		...
		{"case 04", args{2}, true, false, &intGuard},
		{"case 04", args{0}, false, true, &intGuard},
	}
...
func Test_SimpleGuardGT_GetConstraint(t *testing.T) {
	intRef := g.NewSimpleReference[int]()
	intRef.Set("key1", 1)
	emptyRef := g.NewSimpleReference[int]()
	intGuard := g.NewSimpleGuardGT[int]("key1", intRef)
