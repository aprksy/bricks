...
func Test_NewSimpleGuardNE(t *testing.T) {
	type args struct {
		id        string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			instance := g.NewSimpleGuardNE(tt.args.id, tt.args.reference)
			a.NotNil(t, instance, "instance should not be nil")
		})
	}
}

func Test_SimpleGuardNE_Evaluate(t *testing.T) {
	type args struct {
		value int
	}
	...
	intGuard := g.NewSimpleGuardNE[int]("key1", intRef)
	emptyGuard1 := g.NewSimpleGuardNE[int]("key1", nil)
	emptyGuard2 := g.NewSimpleGuardNE[int]("key1", emptyRef)

	testCases := []struct {
		name     string
		args     args
		expected bool
		guard    *g.SimpleGuardNE[int]
	}{
...
func Test_SimpleGuardNE_EvaluateWithErr(t *testing.T) {
	type args struct {
		value int
	}
...
func Test_SimpleGuardNE_GetConstraint(t *testing.T) {
	intRef := g.NewSimpleReference[int]()
	intRef.Set("key1", 1)
	emptyRef := g.NewSimpleReference[int]()
	intGuard := g.NewSimpleGuardNE[int]("key1", intRef)
